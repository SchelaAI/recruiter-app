...
This build includes request-scoped auth/profile memoization, count-only dashboard metrics, bounded conversation history, reduced candidate lookup payloads, batched realtime refreshes, reusable server SDK clients, optimized formatter reuse, and the Schela production favicon/logo. Calendly callback/webhook origins are also forced to HTTPS outside local development.
