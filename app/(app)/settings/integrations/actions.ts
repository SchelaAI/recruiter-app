...
import { createCalendlyWebhookSubscription, deleteCalendlyWebhookSubscription, getCalendlyConnection, listCalendlyEventTypes } from "@/lib/calendly/client";
...
export async function repairCalendlyWebhook() {
  const { profile } = await requireAppUser();
  const orgId = profile.org_id!;
  const admin = createAdminClient();
  const connection = await getCalendlyConnection(orgId);
  if (!connection) redirect("/settings/integrations?error=Connect%20Calendly%20first.");

  try {
    const publicOrigin = await getRequestOrigin();
    const webhook = await createCalendlyWebhookSubscription({
      orgId,
      callbackUrl: `${publicOrigin}/api/webhooks/calendly`,
    });
    ...
    const [connectionUpdate, integrationUpdate] = await Promise.all([
      ...
      admin.from("integrations").upsert({
        ...
        icon: "calendar_month",
        connected: true,
        account: connection.account_email || connection.account_name || "Connected",
        last_synced: now,
        config: null,
      }, { onConflict: "id,org_id" }),
    ]);
    if (connectionUpdate.error) throw new Error(connectionUpdate.error.message);
    if (integrationUpdate.error) throw new Error(integrationUpdate.error.message);
  } catch (error) {
    await recordOperationalEvent({
      severity: "error",
      source: "calendly_oauth",
      eventType: "webhook_repair_failed",
      orgId,
      message: error instanceof Error ? error.message : "Calendly webhook repair failed",
    });
    redirect("/settings/integrations?error=Could%20not%20repair%20Calendly%20webhook.%20Reconnect%20Calendly%20to%20grant%20the%20latest%20permissions.");
  }

  revalidatePath("/settings/integrations");
  redirect("/settings/integrations?repaired=1");
}
