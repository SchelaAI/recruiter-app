...
export async function updateCompany(formData: FormData) {
  ...
  const companyName = name.slice(0, 160);
  const now = new Date().toISOString();
  const [organizationUpdate, profileUpdate] = await Promise.all([
    admin.from("organizations").update({ name: companyName, website, updated_at: now }).eq("id", orgId),
    admin.from("profiles").update({ company: companyName, updated_at: now }).eq("org_id", orgId),
  ]);
  const error = organizationUpdate.error ?? profileUpdate.error;
  if (error) {
