...
import { cleanInboundEmailText } from "@/lib/email/content";

function messageDisplayText(message: { text: string; channel: string; from_role: string }) {
  if (message.channel === "em" && message.from_role === "candidate") {
    return cleanInboundEmailText(message.text) || "Email reply";
  }
  return message.text;
}
...
export default async function ConversationPage({ params, searchParams }: { params: Promise<{ conversationId: string }>; searchParams: Promise<{ outreach?: string; sent?: string; error?: string; deliveryError?: string }> }) {
  ...
  const [candidateResult, messagesResult, interviewResult, allCandidatesResult, listMessagesResult] = await Promise.all([
    ...
    conversationIds.length ? supabase.from("messages").select("conversation_id,text,channel,from_role,created_at").eq("org_id", orgId).in("conversation_id", conversationIds).order("created_at", { ascending: false }).limit(200) : Promise.resolve({ data: [] as Array<{conversation_id:string;text:string;channel:string;from_role:string;created_at:string}> }),
  ]);
  ...
  const latest = new Map<string, { text:string; channel:string; from_role:string; created_at:string }>();
  ...
  return <div className="workspace-page conversation-page">
    ...
    <section className="conversation-workspace">
      <aside className="conversation-list-pane">
        <div className="conversation-search"><Icons.search size={17}/><span>Search conversations...</span></div>
        <div className="conversation-list">{(conversations ?? []).map((row) => { const name = names.get(row.candidate_id) ?? row.candidate_id; const msg = latest.get(row.id); return <Link href={`/inbox/${row.id}`} key={row.id} className={`conversation-list-row ${row.id === conversationId ? "active" : ""}`}><span className="square-avatar">{initials(name)}</span><span className="conversation-list-copy"><b>{name}</b><small>{msg ? messageDisplayText(msg) : "Conversation ready"}</small></span><time>{formatRelativeTime(msg?.created_at ?? row.updated_at)}</time></Link>; })}</div>
      </aside>
