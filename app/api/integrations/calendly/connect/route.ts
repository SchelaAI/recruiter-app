...
export async function GET(request: NextRequest) {
  ...
  if (!profile?.org_id || !profile.onboarding_completed) {
    return NextResponse.redirect(new URL("/onboarding", publicOrigin), { headers: noStoreHeaders() });
  }
  ...
  if (!limit.allowed) {
    return NextResponse.redirect(
      new URL("/settings/integrations?error=Too%20many%20Calendly%20connection%20attempts.%20Please%20try%20again%20later.", publicOrigin),
