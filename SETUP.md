...
2. Run each migration in order, pasting the full contents of each file and running it:
   ...
   - `supabase/migrations/0016_fix_candidate_conversation_pk.sql` — **fixes a real bug**: `candidates.id` and `conversations.id` were globally-unique text keys shared across every org, so an inbound reply could silently attach to a different org's conversation row (message stored, webhook returns 200, never appears in the thread). Makes both keys `(id, org_id)` and adds `messages.org_id`.
   - `supabase/migrations/0017_calendly_cancel_reschedule.sql` — adds `interviews.calendly_event_uri`, needed for in-app cancel/reschedule to actually reach Calendly's API instead of only updating Schela's own row (see §11b).
   - `supabase/migrations/0018_noreply_email_followup.sql` — adds `interviews.followup_email_sent_at`, powering the 24-hour no-reply cross-channel follow-up (WhatsApp invite unanswered → follow up by email). See §13.
   - `supabase/migrations/0019_whatsapp_delivery_status.sql` — adds `messages.whatsapp_message_id`, needed to track REAL WhatsApp delivery status. Meta's send API only confirms "accepted", not "delivered" — actual delivery/failure arrives later via a separate status webhook, which Schela previously discarded entirely.
   - `supabase/migrations/0020_email_threading.sql` — adds `messages.email_message_id`. Fixes escalation (and every other email flow) starting a new thread instead of continuing the existing one — no outbound email previously set the In-Reply-To/References headers mail clients actually thread on.
...
WhatsApp is **centralized**: one number for the whole deployment, set once
via env vars. Every org sends and receives through this same number — there
is no per-org "connect your own WhatsApp" flow. This is deliberate: it means
a recruiter signing up to Schela never needs a Meta developer account,
Phone Number ID, or access token of their own.
...
3. A candidate's `phone` + `country_code` in your `candidates` table must
   ...
   knows which candidate (and therefore which org) an inbound message
   belongs to, and it's global by design: the same webhook, the same number,
   matching across every org's candidates on the deployment.
...
the Integrations page is backed by a real connection — WhatsApp centralized
via env vars (see §9); Resend via validated, org-stored credentials;
Outlook, Zoom, and Calendly via real OAuth (see §11, §11b). Email sending
...
`/api/cron/reminders` runs two independent jobs on each call:

1. **Interview reminders** — 24-hour and 1-hour reminders before a scheduled
   interview.
2. **No-reply follow-ups** — if a candidate hasn't replied to their WhatsApp
   invitation within 24 hours, Schela follows up by **email** instead of
   repeating itself on a channel they clearly aren't reading. The
   conversation switches to email from that point, so when they reply the
   AI continues the thread there automatically.

Both are stamped per-interview, so overlapping runs can't double-message.
The original description: It's a plain authenticated GET, so **any** scheduler that can make
