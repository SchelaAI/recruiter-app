...
async function listCalendlyWebhookSubscriptions(
  orgId: string,
  connection: NonNullable<Awaited<ReturnType<typeof getCalendlyConnection>>>,
  scope: "organization" | "user",
) {
  ...
  if (scope === "user") params.set("user", connection.owner_uri);

  const result = await calendlyFetch<{ collection: CalendlyWebhookSubscription[] }>(
    orgId,
...
async function findExistingCalendlyWebhookSubscription(input: {
  ...
  const subscriptions = await listCalendlyWebhookSubscriptions(input.orgId, input.connection, input.scope);
  return subscriptions.find((subscription) => {
    if (subscription.callback_url !== input.callbackUrl) return false;
    const events = new Set(subscription.events ?? []);
    return events.has("invitee.created") && events.has("invitee.canceled");
  }) ?? null;
}

async function createWebhookForScope(input: {
  orgId: string;
  callbackUrl: string;
  connection: NonNullable<Awaited<ReturnType<typeof getCalendlyConnection>>>;
  scope: "organization" | "user";
  ...
  const body: Record<string, unknown> = {
    ...
    organization: input.connection.organization_uri,
    scope: input.scope,
  };
  if (input.scope === "user") body.user = input.connection.owner_uri;

  try {
    const result = await calendlyFetch<{ resource: CalendlyWebhookSubscription }>(input.orgId, "/webhook_subscriptions", {
      method: "POST",
    ...
    // A prior OAuth attempt may have created the remote subscription and then
    // failed before Schela persisted its URI. Recover that subscription rather
    // than leaving the integration permanently half-connected.
    const existing = await findExistingCalendlyWebhookSubscription(input);
...
export async function createCalendlyWebhookSubscription(input: {
  ...
  try {
    return await createWebhookForScope({
      ...input,
      connection,
      scope: "organization",
    });
    ...
    // Non-admin Calendly users cannot create organization-wide subscriptions.
    // A user-scoped subscription is sufficient for that user's own event types.
    return createWebhookForScope({
      ...input,
      connection,
      scope: "user",
    });
...
export async function createInterviewSchedulingLink(input: {
  ...
  if (interviewUpdateError) {
    throw new Error(`Could not mark Calendly link sent: ${interviewUpdateError.message}`);
  }
