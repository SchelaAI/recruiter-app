...
/**
 * Cancels a real Calendly booking. This is what makes in-app cancellation
 * actually reach Calendly — without it, cancelling an interview in Schela
 * only sent the candidate a message; the Calendly event (and the real
 * calendar event underneath it) stayed live.
 */
export async function cancelCalendlyEvent(
  ...
  try {
    // eventUri is a full resource URI (https://api.calendly.com/scheduled_events/UUID);
    // the cancellation endpoint hangs off that same path.
    const res = await fetch(`${eventUri}/cancellation`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
      body: JSON.stringify({ reason: reason || "Cancelled by recruiter" }),
    });
    if (!res.ok) {
      const json = await res.json().catch(() => ({}));
      console.error("[calendly] cancellation failed:", JSON.stringify(json));
      return { ok: false, error: json?.message ?? `HTTP ${res.status}` };
    }
    return { ok: true };
  } catch (err) {
