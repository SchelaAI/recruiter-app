...
export async function createCandidate(input: Partial<Candidate>): Promise<Candidate> {
  ...
  const idPrefix =
    (input.name ?? "New Candidate")
      ...
      .toUpperCase()
      .slice(0, 2) || "NC";
  // Retry on collision. IDs are only ~90 values per initial-pair, so even
  // within a single org duplicates are likely once a few candidates share
  // initials — without this, creating a candidate just throws.
  let id = `${idPrefix}${Math.floor(Math.random() * 90 + 10)}`;
  for (let attempt = 0; attempt < 12; attempt++) {
    const { data: clash } = await supabase
      .from("candidates").select("id").eq("id", id).eq("org_id", orgId).maybeSingle();
    if (!clash) break;
    id = `${idPrefix}${Math.floor(Math.random() * 90 + 10)}`;
  }
  ...
  if (candidate) {
    ...
    if (row.channel === "wa") {
      ...
      const res = await sendEmailForOrg(
        orgId, candidate.email, `Your interview — ${input.jobPosition}`, invitation, { conversationId }
      );
      delivered = res.ok;
      deliveryError = res.error;
    ...
    await supabase
      .from("conversations")
      .upsert({ id: conversationId, org_id: orgId, candidate_id: candidate.id, channel: row.channel }, { onConflict: "id,org_id" });
...
export async function rescheduleInterview(id: number, newTimeISO: string): Promise<{ interview: Interview | null; delivered: boolean; deliveryError?: string }> {
  ...
  const { data: before } = await supabase
    .from("interviews").select("format, calendly_event_uri").eq("id", id).eq("org_id", orgId).single();
  const isCalendly = before?.format === "Calendly";

  // For a Calendly interview, the picked newTimeISO is only ever a
  // placeholder — same as on creation — because the whole point of Calendly
  // is the candidate picks their own time. Cancel the old booking (if one
  // exists) so it doesn't sit live on the calendar next to a new one, and
  // send a fresh link instead of proposing this specific time via text,
  // which would contradict the self-service model entirely.
  let calendlyBookingUrl: string | undefined;
  if (isCalendly) {
    const calendlyToken = await getOAuthAccessTokenForOrg(orgId, "calendly");
    if (calendlyToken) {
      if (before?.calendly_event_uri) {
        const { cancelCalendlyEvent } = await import("@/lib/integrations/calendly");
        const cancelResult = await cancelCalendlyEvent(calendlyToken, before.calendly_event_uri, "Rescheduled by recruiter");
        if (!cancelResult.ok) console.error(`[rescheduleInterview] couldn't cancel prior Calendly booking for interview ${id}:`, cancelResult.error);
      }
      const { data: calendlyRow } = await supabase.from("integrations").select("config").eq("org_id", orgId).eq("id", "calendly").single();
      const eventTypeUri = (calendlyRow?.config as { event_type_uri?: string } | null)?.event_type_uri;
      if (eventTypeUri) {
        const { createCalendlySchedulingLink } = await import("@/lib/integrations/calendly");
        const link = await createCalendlySchedulingLink(calendlyToken, eventTypeUri);
        if (link.ok) calendlyBookingUrl = link.bookingUrl;
        else console.error(`[rescheduleInterview] couldn't create a new Calendly link for interview ${id}:`, link.error);
      }
    }
  }
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      ...(isCalendly ? { calendly_event_uri: null, meeting_link: calendlyBookingUrl ?? null } : {}),
    })
    .eq("id", id)
    .eq("org_id", orgId)
  ...
  if (candidate) {
    const firstName = candidate.name.split(" ")[0];
    const text = isCalendly
      ? (calendlyBookingUrl
          ? `Hi ${firstName},\n\nWe need to reschedule your interview. Please pick a new time that works for you:\n\n${calendlyBookingUrl}\n\nOnce you've booked, I'll confirm here.`
          : `Hi ${firstName},\n\nWe need to reschedule your interview — I'm sorting out a new booking link and will send it shortly.`)
    ...
    if (row.channel === "wa") {
      const sendResult = await sendWhatsAppWithTemplateFallback(
        orgId, `${candidate.country_code ?? ""}${candidate.phone}`, text, [firstName, isCalendly ? "a new time" : slotLabel]
      );
      if (!sendResult.ok) { delivered = false; deliveryError = sendResult.error; deliveryFailed = true; }
      whatsappMessageId = sendResult.messageId;
    } else {
      const emailResult = await sendEmailForOrg(orgId, candidate.email, "Rescheduling your interview", text, { conversationId });
    ...
    await supabase.from("conversations").update({ updated_at: new Date().toISOString() }).eq("id", conversationId).eq("org_id", orgId);
  }
...
export async function listConversations(): Promise<Conversation[]> {
  ...
  // Fetch every message for this org's conversations in ONE query, then
  // group in memory. This was previously a query per conversation inside a
  // loop — a classic N+1 that ran sequentially, so 30 conversations meant 31
  // round-trips one after another and a visibly slow Conversations page.
  const conversationIds = conversations.map((c) => c.id);
  const messagesByConversation = new Map<string, Message[]>();

  if (conversationIds.length > 0) {
    const { data: allMsgRows, error: msgError } = await supabase
      .from("messages")
      .select("*")
      .eq("org_id", orgId)
  ...
  if (from === "schela") {
    if (conv.channel === "wa") {
      ...
      if (candidate) {
        ...
        const sendResult = await sendWhatsAppMessage(`${candidate.country_code ?? ""}${candidate.phone}`, text);
        if (!sendResult.ok) {
          delivered = false;
      ...
      const { data: candidate } = await supabase
        ...
        .single();
      if (candidate?.email) {
        const emailResult = await sendEmailForOrg(orgId, candidate.email, "Regarding your interview", text, { conversationId });
        if (!emailResult.ok) {
          delivered = false;
 ...
 */
export async function receiveInboundMessageForOrg(
  orgId: string,
  candidateId: string,
  text: string,
  channel: "wa" | "em",
  /** The inbound email's real Message-ID (from Resend's webhook payload), stored so Schela's reply can correctly reference it in References. Irrelevant for WhatsApp. */
  emailMessageId?: string
) {
  const admin = createAdminClient();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return processInbound(admin as any, orgId, candidateId, text, channel, emailMessageId);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function processInbound(client: any, orgId: string, candidateId: string, text: string, channel: "wa" | "em", inboundEmailMessageId?: string) {
  ...
  // Supabase returns { error } rather than throwing, so an unchecked upsert
  // fails SILENTLY — and then the message insert below hits a foreign-key
  // violation, throws, gets swallowed by the webhook's catch, and the whole
  // thing returns 200 with the candidate's reply nowhere to be seen. Check it.
  const convRow = { id: conversationId, org_id: orgId, candidate_id: candidateId, channel };
  let { error: convError } = await client.from("conversations").upsert(convRow, { onConflict: "id,org_id" });

  // The composite conflict target only exists once 0016 has been applied.
  // On a database still running the old single-column primary key, Postgres
  // rejects it ("no unique or exclusion constraint matching the ON CONFLICT
  // specification") — so fall back rather than losing the message purely
  // because migrations and code were deployed in a different order.
  if (convError && /ON CONFLICT|constraint matching/i.test(convError.message ?? "")) {
    console.warn("[processInbound] composite conflict target unavailable — falling back to id. Run migration 0016.");
    ({ error: convError } = await client.from("conversations").upsert(convRow, { onConflict: "id" }));
  }

  if (convError) {
    console.error(
      `[processInbound] could not upsert conversation ${conversationId} for org ${orgId}:`,
      convError.message ?? convError
    );
    throw convError;
  }
  ...
  const inboundRow = {
    conversation_id: conversationId, org_id: orgId, from_role: "candidate", text, channel,
    sender_kind: "candidate", sender_name: candidate.name,
    email_message_id: channel === "em" ? (inboundEmailMessageId ?? null) : null,
  };
  let { data: msgRow, error: msgError } = await client.from("messages").insert(inboundRow).select().single();

  // messages.org_id also only exists after 0016 — same deploy-order guard as
  // the conversation upsert above, so a reply is never lost to migration lag.
  if (msgError && /org_id/i.test(msgError.message ?? "")) {
    console.warn("[processInbound] messages.org_id unavailable — inserting without it. Run migration 0016.");
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { org_id: _omit, ...legacyRow } = inboundRow;
    ({ data: msgRow, error: msgError } = await client.from("messages").insert(legacyRow).select().single());
  }

  if (msgError) {
    console.error(
      `[processInbound] could not insert inbound message into ${conversationId}:`,
      msgError.message ?? msgError
    );
    throw msgError;
  }
  console.log(`[processInbound] stored inbound ${channel} message from candidate ${candidateId} in ${conversationId}`);
  ...
  if (result.action === "auto_reply" && result.draftReply) {
    ...
    if (channel === "wa") {
      ...
      const sendResult = await sendWhatsAppMessage(phone, result.draftReply);
      if (!sendResult.ok) {
      ...
      const emailResult = await sendEmailForOrg(
        orgId, candidate.email, `Re: your interview — ${candidate.job_position ?? "your application"}`, result.draftReply,
        { conversationId }
      );
      if (!emailResult.ok) {
        deliveryFailed = true;
...
export async function listIntegrations(): Promise<Integration[]> {
  ...
  return rows.map((i) => {
    ...
    if (i.id === "whatsapp") {
      // Centralized: one WhatsApp number for the whole deployment, loaded
      // from WHATSAPP_PHONE_NUMBER_ID / WHATSAPP_ACCESS_TOKEN. Every org
      // shows the same connected state — there's no per-org credential to
      // check anymore.
      return {
        id: i.id, name: i.name, icon: i.icon, connected: envWhatsAppConfigured,
        account: envWhatsAppConfigured ? "Configured via environment variables" : undefined,
...
export async function sendEmailForOrg(
  ...
  opts?: {
    ...
    /**
     * When given, threads this send into the existing conversation using
     * real In-Reply-To/References headers (see lib/integrations/resend.ts)
     * instead of starting a new thread in the candidate's inbox. This is
     * the actual fix for "escalation starts a new email thread" — and every
     * other email flow had the identical gap, since none of them set these
     * headers before. Pass the conversation's id for every email tied to an
     * ongoing candidate conversation.
     */
    conversationId?: string;
  }
  ...
  if (opts?.conversationId) {
    // Full chain, oldest first — References carries every prior Message-ID
    // in the thread, In-Reply-To is just the most recent one. Both inbound
    // and outbound messages contribute, since a thread is bidirectional.
    ...
    const ids = (priorEmails ?? []).map((m) => m.email_message_id).filter((id): id is string => Boolean(id));
    if (ids.length > 0) {
      references = ids;
      inReplyTo = ids[ids.length - 1];
      // Headers are the reliable mechanism, but keep the subject consistent
      // too — it's what a human scanning their inbox actually reads, and
      // it's the fallback every mail client still uses if headers are ever
      // stripped by an intermediate relay.
  ...
  return sendEmail({
    apiKey, to: toEmail, from: fromAddress, fromName, replyTo,
    subject: finalSubject, text, attachment: opts?.attachment,
...
export async function notifyCancellation(interviewId: number, reason: string): Promise<{ notified: boolean; error?: string }> {
  ...
  const { data: ivRaw } = await supabase
    .from("interviews")
    .select("channel, scheduled_at, format, calendar_event_id, calendly_event_uri, candidates(id, name, phone, country_code, email)")
    .eq("id", interviewId)
    .eq("org_id", orgId)
    .single();

  if (!ivRaw) return { notified: false, error: "Interview not found" };
  const iv = ivRaw as unknown as {
    channel: "wa" | "em"; scheduled_at: string; format: string;
    calendar_event_id: string | null; calendly_event_uri: string | null;
  ...
  // Cancel the REAL booking, not just Schela's own row. Previously this
  // function only ever messaged the candidate — the actual Calendly booking,
  // or the actual Outlook calendar event, stayed live regardless.
  if (iv.format === "Calendly" && iv.calendly_event_uri) {
    const calendlyToken = await getOAuthAccessTokenForOrg(orgId, "calendly");
    if (calendlyToken) {
      const { cancelCalendlyEvent } = await import("@/lib/integrations/calendly");
      const result = await cancelCalendlyEvent(calendlyToken, iv.calendly_event_uri, reason);
      if (!result.ok) console.error(`[notifyCancellation] Calendly cancellation failed for interview ${interviewId}:`, result.error);
    }
  } else if (iv.calendar_event_id) {
    const outlookToken = await getOAuthAccessTokenForOrg(orgId, "outlook");
    if (outlookToken) {
      const { deleteOutlookEvent } = await import("@/lib/integrations/outlook");
      const result = await deleteOutlookEvent(outlookToken, iv.calendar_event_id);
      if (!result.ok) console.error(`[notifyCancellation] Outlook event deletion failed for interview ${interviewId}:`, result.error);
    }
  }
  ...
  if (iv.channel === "wa") {
    const { sendWhatsAppMessage } = await import("@/lib/integrations/whatsapp");
    result = await sendWhatsAppMessage(`${candidate.country_code ?? ""}${candidate.phone}`, text);
  } else {
    result = await sendEmailForOrg(orgId, candidate.email, "Your interview has been cancelled", text, { conversationId });
  }
  ...
  await supabase.from("conversations").upsert({ id: conversationId, org_id: orgId, candidate_id: candidate.id, channel: iv.channel }, { onConflict: "id,org_id" });
  await supabase.from("messages").insert({
    conversation_id: conversationId,
    org_id: orgId,
    from_role: "schela",
    ...
    whatsapp_message_id: iv.channel === "wa" ? (result.messageId ?? null) : null,
    email_message_id: iv.channel === "em" ? (result.messageId ?? null) : null,
    sender_kind: "ai",
    sender_name: "Schela",
  });
...
export async function sendReminders(): Promise<{ sent: number; failed: number; names: string[] }> {
  ...
  for (const c of candidates) {
    ...
    await supabase
      .from("conversations")
      .upsert({ id: conversationId, org_id: orgId, candidate_id: c.id, channel }, { onConflict: "id,org_id" });
    ...
    if (channel === "wa") {
      ...
      const res = await sendEmailForOrg(orgId, c.email, `Your interview invitation — ${role}`, text, { conversationId });
      ok = res.ok;
      if (!res.ok) deliveryErrorMsg = res.error;
      emailMessageId = res.messageId;
    }
...
export async function sendAttachment(
  ...
  if (conv.channel === "wa") {
    const { data: candidate } = await supabase
      .from("candidates").select("phone, country_code").eq("id", conv.candidate_id).eq("org_id", orgId).single();
    if (candidate) {
      const { sendWhatsAppMedia } = await import("@/lib/integrations/whatsapp");
      const res = await sendWhatsAppMedia(
        `${candidate.country_code ?? ""}${candidate.phone}`, upload.url, mime, filename, caption
      );
    ...
    const { data: candidate } = await supabase
      .from("candidates").select("email").eq("id", conv.candidate_id).eq("org_id", orgId).single();
    if (candidate?.email) {
      const base64 = Buffer.from(bytes).toString("base64");
      const res = await sendEmailForOrg(
        orgId, candidate.email, caption?.trim() || `Attachment: ${filename}`, caption?.trim() || "Please find the attached file.",
        { attachment: { base64, filename, mime }, conversationId }
      );
      if (!res.ok) { delivered = false; deliveryError = res.error; }
      emailMessageId = res.messageId;
    } else {
...
async function sendWhatsAppWithTemplateFallback(
...
): Promise<{ ok: boolean; error?: string; usedTemplate?: boolean; messageId?: string }> {
  const { sendWhatsAppMessage, sendWhatsAppTemplate, OUTSIDE_WINDOW_ERROR_CODES } = await import("@/lib/integrations/whatsapp");

  const direct = await sendWhatsAppMessage(toPhone, text);
  if (direct.ok) return { ok: true, messageId: direct.messageId };
  ...
  return { ok: true, usedTemplate: true, messageId: result.messageId };
}
...
export async function sendDueReminders(): Promise<{ sent24h: number; sent1h: number; failed: number }> {
  ...
  for (const raw of interviews ?? []) {
    ...
    if (iv.channel === "wa") {
      ...
      const res = await sendEmailForOrg(iv.org_id, candidate.email, "Interview reminder", text, { conversationId });
      ok = res.ok;
      emailMessageId = res.messageId;
...
/* ============ 24-HOUR NO-REPLY → EMAIL FALLBACK ============ */

/**
 * Follows up by EMAIL when a candidate hasn't replied to their invitation
 * within 24 hours.
 *
 * The cross-channel switch is the point: repeating the message on WhatsApp
 * when they're plainly not reading WhatsApp achieves nothing, so the retry
 * goes out on the other channel. Once they reply to that email, the normal
 * inbound webhook picks it up and the AI carries the conversation on exactly
 * as it would have on WhatsApp — same thread, same conversation row.
 *
 * Idempotent: each send stamps followup_email_sent_at and only null-stamped
 * interviews are selected, so overlapping cron runs can't double-message.
 */
export async function sendNoReplyFollowUps(): Promise<{ sent: number; failed: number; skipped: number }> {
  const admin = createAdminClient();
  const cutoff = new Date(Date.now() - 24 * 60 * 60_000).toISOString();

  // Candidate hasn't engaged yet: still awaiting a reply, invitation went out
  // more than 24h ago, and no cross-channel follow-up sent yet.
  const { data: interviews, error } = await admin
    .from("interviews")
    .select("id, org_id, channel, candidate_id, created_at, candidates(id, name, email, job_position)")
    .in("ai_state", ["sending_invitation", "waiting_reply"])
    .is("followup_email_sent_at", null)
    .lte("created_at", cutoff)
    .limit(100);

  if (error) {
    console.error("[followups] interview lookup failed:", error.message);
  ...
  for (const raw of interviews ?? []) {
    const iv = raw as unknown as {
      id: number; org_id: string; channel: "wa" | "em"; candidate_id: string; created_at: string;
      candidates: { id: string; name: string; email: string; job_position: string | null } | null;
    };
    const candidate = iv.candidates;
    ...
    // Authoritative check: has the candidate ACTUALLY sent anything? ai_state
    // can lag behind reality, and chasing someone who already replied is
    // exactly the kind of thing that makes an automated recruiter look broken.
    const { count: replyCount } = await admin
      .from("messages")
      .select("id", { count: "exact", head: true })
      .eq("conversation_id", conversationId)
      .eq("org_id", iv.org_id)
      .eq("from_role", "candidate");

    if ((replyCount ?? 0) > 0) {
      // They replied — stamp it so this interview stops being rescanned every hour.
      await admin.from("interviews").update({ followup_email_sent_at: new Date().toISOString() }).eq("id", iv.id);
      skipped++;
      continue;
    }

    const firstName = candidate.name.split(" ")[0];
    const role = candidate.job_position ?? "the role";
    const text =
      `Hi ${firstName},\n\n` +
      `Just checking in regarding your interview invitation for ${role}.\n\n` +
      `I reached out on WhatsApp but haven't heard back, so I'm following up here instead.\n\n` +
      `If you're still interested, simply reply to this email with your preferred interview slot.`;

    const emailResult = await sendEmailForOrg(iv.org_id, candidate.email, `Following up — ${role}`, text, { conversationId });
    if (!emailResult.ok) {
      console.error(`[followups] email follow-up failed for interview ${iv.id}:`, emailResult.error);
      failed++;
    } else {
      sent++;
    }

    // Stamp regardless of delivery outcome — a failing send retried every
    // hour would spam the candidate the moment it started working.
    await admin.from("interviews").update({ followup_email_sent_at: new Date().toISOString() }).eq("id", iv.id);

    // The conversation switches to email from here, so replies land on the
    // right channel and Schela's own next reply goes back out over email too.
    await admin.from("conversations").upsert(
      { id: conversationId, org_id: iv.org_id, candidate_id: candidate.id, channel: "em" },
      { onConflict: "id,org_id" }
    );
    await admin.from("conversations").update({ channel: "em", updated_at: new Date().toISOString() })
      .eq("id", conversationId).eq("org_id", iv.org_id);
    ...
    console.log(`[followups] emailed follow-up for interview ${iv.id} to ${candidate.email} (delivered: ${emailResult.ok})`);
  }
